...
#         print(items)
    ...
    #             print('Task Last Activity: %s' % task_last_activity)
                ...
                #test = [card_name, assigned_member_ids, assigned_member_names, task_category, task_duration, task_description, task_last_activity]
    #             df = pd.DataFrame(task_details)
    #             new = df.T

    #             new.rename(columns = {new.columns[0]: "card_name", new.columns[1]: "assigned_member_ids", new.columns[2]: "ssigned_member_names", 
    #                                   new.columns[3]: "task_category", new.columns[4]: "task_duration",
    #                                   new.columns[5]: "task_description", new.columns[6]: "task_last_activity" }, inplace = True )
                df.append(task_details, ignore_index=True)

    #             new.to_csv('trello.csv', index=False)



        #         print(task_description)
        #         print(task_last_activity)
        #         print(card._json_obj)
                print()
            except Exception as e:
                print(e)
...
#     df = df[['labels', 'card_name', 'assigned_member_names', 'task_category', 'task_duration', 'task_last_activity', 'due_date']]
...
#     label_ = df['labels'].unique()
#     Sprint 1.1
#     Sprint 1.1 In Progress
#     Sprint 1.1 in Review
#     Sprint 1.1 Completed
    current_sprint = 'Sprint 1.1'
    progress = 'Sprint 1.1 In Progress'
    review = 'Sprint 1.1 in Review'
    completed_task = 'Sprint 1.1 Completed'
    alpha_complete = 'Sprint Alpha Completed'
    blocker = 'Blocker'
...
#         'response' : texts,
    ...
    title = 'Sprint 1.1 Alerts'
#     sprint_body = {
#         'Task in current_sprint':sprint_1_1,
#         'Number of task in current_sprint':number_of_task_sprint,
#         'current_sprint task duration':total_duration_sprint,
#         'Users with no task':unassign_users_dicts
#     }
...
#         'response' : text,
    ...
    unassign_title = 'Members without Task currently'
...
#     progress_dict = [v for k, v in progress_dict.items()]
    ...
    progress['Overdue_since'] = overdue.astype('timedelta64[D]').astype(int)
...
#         'Number of task(per user) in review':progress_count,
#         'Review task(per user) duration ':progress_duration,
#         'Users without any task': unassign_users_dict
#     }
    
#     progress_alert = trello_alert(progress_title, progress_body)
...
#         'Number of task(per user) in review':review_count,
#         'Review task(per user) duration ':review_duration
#     }
...
#     complete = [v for k, v in complete.items()]
    complete_title = 'Completed Task Alerts'
#     complete_body = {
#         'Completed task':complete,
#         'Number of task(per user) completed':complete_count
#     }
    ...
    #sprint alhpha complete category
    complete_alpha = df[df['labels'] == alpha_complete]
    complete_alpha = complete_alpha[['assigned_member_names', 'card_name', 'task_category', 'task_duration']]
    complete_alpha_count = complete_alpha['assigned_member_names'].value_counts().to_dict()
    
    complete_alpha = convert_df_dict(complete_alpha)
#     complete = [v for k, v in complete.items()]
    complete_alpha_title = 'Completed Alpha Task Alerts'
#     complete_body = {
#         'Completed task':complete,
#         'Number of task(per user) completed':complete_count
#     }
    
    complete_alpha_count_title = 'Number of task completed by each member for sprint alpha'
    complete_alpha_alert = trello_alert(complete_alpha_title, complete_alpha)
    complete_alpha_alert = trello_alert(complete_count_title, complete_alpha_count)
    ...
    final_result = {
      ...
      'unassign_users_dict':unassign_users_dict, 
      'number_of_task_sprint':number_of_task_sprint, 
      'total_duration_sprint':total_duration_sprint, 
      'Overdue_progress':Overdue_progress
    }
    return final_result
